...
def render(df: pd.DataFrame, session) -> None:
    ...
    if hits.empty:
        state.set_selected(session, None); return
    row = hits.iloc[0]
    with st.expander(f"📋 Entity details — {row.get(Col.BRAND, '—')}", expanded=True):
        ...
        if st.button("Close", key="drawer_close"):
            state.set_selected(session, None); st.rerun()
...
def _render_header(row: pd.Series) -> None:
    ...
    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:start;"><div><div style="font-size:20px;font-weight:800;">{brand}</div><div style="color:var(--muted);font-size:12px;">{service} · {regulator}</div></div><div>{risk_badge_html(level)}</div></div>', unsafe_allow_html=True)
...
def _render_rationale(row: pd.Series) -> None:
    ...
    if snippet:
        st.markdown("**Key snippet**"); st.caption(str(snippet))
    if url:
...
def _render_workflow(row: pd.Series, session) -> None:
    ...
    new_status = st.radio("Status", options=_WORKFLOW_STATUSES, index=_WORKFLOW_STATUSES.index(current) if current in _WORKFLOW_STATUSES else 0, horizontal=True, key=f"drawer_workflow_{entity_id}", label_visibility="collapsed")
    if new_status != current:
        state.set_workflow(session, entity_id, new_status); st.success(f"Marked as {new_status}")
...
def _render_annotations(row: pd.Series, session) -> None:
    ...
    if not notes: st.caption("No annotations yet.")
    note = st.text_input("Add a note", key=f"drawer_note_{entity_id}", placeholder="Type and press Enter")
    if note:
        state.add_annotation(session, entity_id, note); st.success("Annotation saved"); st.rerun()
