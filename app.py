...
# ── NOISE FILTER ─────────────────────────────────────────────────────────
# Entities whose normalised brand matches any of these are stripped from results
NOISE_BRANDS = {
    "rulebook", "the complete rulebook", "licensing", "centralbank",
    "globenewswire", "globe newswire", "cbuae rulebook", "insights for businesses",
    "2025 mobile development", "transforming payments", "2025 mobile",
    "mobile development", "page", "home", "about", "contact",
    "terms", "privacy policy", "cookie policy", "fintech news",
    "press release", "media release", "news release", "blog post",
    "white paper", "report", "research", "survey", "study",
    "conference", "event", "webinar", "podcast", "linkedin",
    "twitter", "facebook", "instagram", "youtube",
}

NOISE_PATTERNS = re.compile(
    r"^(\d+[\.\)]|top \d+|best \d+|leading \d+|guide|how to|what is|"
    r"list of|complete list|overview|introduction|insights for|"
    r"transforming|mobile development|press release|whitepaper)",
    re.I
)
...
def is_noise_brand(brand: str) -> bool:
    ...
    if NOISE_PATTERNS.match(b):
        return True
    # reject very long names (likely a sentence not a brand)
...
def get_suggestions(df: pd.DataFrame, query: str) -> list[str]:
    """Return up to 8 brand suggestions matching the query prefix."""
    if not query or len(query) < 2: return []
    q = query.lower()
    brands = df["Brand"].dropna().unique().tolist()
    # starts-with first, then contains
    starts = [b for b in brands if b.lower().startswith(q)]
    contains = [b for b in brands if q in b.lower() and b not in starts]
    return sorted(starts)[:5] + sorted(contains)[:3]
...
with tab_search:
    ...
    with fc1:
        ...
        raw_search = st.text_input(
            ...
            placeholder="🔍 Type a company name (e.g. Cash, Tabby, Binance...)",
            label_visibility="collapsed",
            key="search_input",
        )
        # Update stored query and reset page when it changes
        if raw_search != st.session_state.search_query:
            st.session_state.search_query = raw_search
            st.session_state.page = 1

        # ── Autocomplete suggestions ──────────────────────────────────
        suggestions = get_suggestions(df, raw_search)
        if suggestions and raw_search:
            st.markdown(
                "<div style='font-size:0.78rem;color:#6B7280;margin:-0.3rem 0 0.3rem 0;'>"
                "💡 Suggestions — click to select:</div>",
                unsafe_allow_html=True)
            sug_cols = st.columns(min(len(suggestions), 4))
            for si, sug in enumerate(suggestions[:4]):
                if sug_cols[si].button(sug, key=f"sug_{si}", use_container_width=True):
    ...
    if st.session_state.search_query:
        filtered = fuzzy_filter(filtered, st.session_state.search_query)
