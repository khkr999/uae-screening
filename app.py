...
def render_overview_priority_card(row: pd.Series) -> None:
    ...
    status = str(row.get("Alert Status", ""))
    status_chip = ""
    if "NEW" in status:
        status_chip = '<span class="overview-status-chip" style="background:#ECFDF3;color:#6FBF7A;border-color:#CFEED8;">NEW</span>'
    elif "INCREASED" in status:
        status_chip = '<span class="overview-status-chip" style="background:#FFF4EA;color:#D0873F;border-color:#F3D5BA;">↑ RISK UP</span>'
    st.markdown(
        f"""
        <div class="overview-priority-card">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;">
            <div style="flex:1;min-width:0;">
              <div class="overview-priority-title">{brand} {status_chip}</div>
...
def render_overview_alerts_card(metrics: dict[str, int]) -> None:
    st.markdown(
        f"""
        <div class="overview-side-card alerts">
          <div class="overview-side-title" style="color:#D0873F;">Alerts This Run</div>
          <div style="color:#6FBF7A;font-size:13px;font-weight:800;margin-bottom:10px;">{metrics["new_entities"]} new entities added</div>
          <div style="color:#D0873F;font-size:13px;font-weight:800;">{metrics["risk_up"]} risk level increases detected</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with k5:
    run_note = f'↑ {metrics["risk_up"]} risk increased' if metrics["risk_up"] else "No new alerts"
    render_overview_kpi("New This Run", str(metrics["new_entities"]), run_note, "#7FE0AE")
...
with tab_home:
    ...
    with right_col:
        render_overview_alerts_card(metrics)
    ...
    if filtered.empty:
        ...
        with detail_col:
            if selected_entity_row is not None:
                render_entity_panel(selected_entity_row, closeable=True, scope="search")
            else:
                st.markdown(
                    f"""
