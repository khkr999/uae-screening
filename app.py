...
with tab_search:
    ...
    # ── Search bar with NATIVE autocomplete dropdown ─────────────────────
    # selectbox searches inside the dropdown exactly like Google's typeahead
    all_brands = ["— All companies —"] + sorted(df["Brand"].dropna().unique().tolist())
    selected_brand = st.selectbox(
        "🔍 Search for a company",
        options=all_brands,
        index=0,
        placeholder="Start typing a company name (e.g. Cash, Tabby, Binance...)",
        help="Type to filter the list — just like Google. Click a name to select it.",
        key="search_selectbox",
    )
    # Clicking a new brand resets to page 1
    ...
    # Brand filter (from autocomplete selectbox)
    if selected_brand and selected_brand != "— All companies —":
