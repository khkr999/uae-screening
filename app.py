...
# ══════════════════════════════════════════════════════════════════════════
_tb_spacer, _tb_toggle = st.columns([6, 1])
with _tb_toggle:
    toggle_label = "☀  Day Mode" if dark else "☾  Night Mode"
    if st.button(toggle_label, use_container_width=True, key="top_theme_toggle",
                 help="Toggle between night and day mode"):
        st.session_state.theme = "light" if dark else "dark"
        st.rerun()
...
with tab_search:
    ...
    if filtered.empty:
        ...
        # Header
        h_cols = st.columns([0.35, 2.6, 0.9, 1.3, 1.1, 0.9, 1.2])
        ...
        for _, row in page_df.iterrows():
            try:
                ...
                rcols = st.columns([0.35, 2.6, 0.9, 1.3, 1.1, 0.9, 1.2])
                with rcols[0]:
                ...
                with rcols[1]:
                    ...
                    cls_short = cls[:60] + "…" if len(cls) > 60 else cls
                    st.markdown(
                ...
                with rcols[4]:
                    short_svc = service if len(service) <= 20 else service[:18] + "…"
