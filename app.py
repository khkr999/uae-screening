...
# Generic financial words that on their own aren't real brand names
GENERIC_ONLY_WORDS = {
...
def is_noise_brand(brand: str) -> bool:
    ...
    # Starts with &, ', ", -, or whitespace-prefixed special char
    ...
    # Only generic words (e.g. "Bank", "Payments")
    words = b.split()
    ...
    if re.search(r"\b(and|or|the|in|on|of|for|with|to|by)\b.*\b(and|or|the|in|on|of|for|with|to|by)\b", b):
        return True

    # Ends with an article (incomplete phrase)
    if b.rstrip('.').split()[-1] in {"the", "a", "an", "of", "in", "on", "for", "to", "and", "or"}:
        return True
