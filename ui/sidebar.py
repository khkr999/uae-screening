...
def render(session) -> Path | None:
    """Return the selected run path, or None if no runs available."""
    with st.sidebar:
        st.markdown("### 🛡️ UAE Screening")
        st.caption("Risk monitoring platform")

        theme_label = ("☀ Light Mode" if session.get("theme") == "dark"
                       else "☾ Dark Mode")
        ...
        if runs:
            ...
            choice = st.selectbox("Pick a run", list(options.keys()),
...
def _render_upload(session) -> None:
    ...
    if uploaded is not None:
        try:
            ...
            # Bump nonce so the uploader clears on rerun
