...
def _render_kpis(m: RunMetrics) -> None:
    ...
    with c1:
        kpi_card("Entities Screened", f"{m.total:,}",
                 "This run", accent="var(--accent)")
    with c2:
        share = f"{round((m.critical_high / max(m.total, 1)) * 100)}% of total"
        kpi_card("Critical / High", m.critical_high, share, accent="#D63C54")
    with c3:
        kpi_card("Needs Review", m.needs_review,
                 "Risk levels 2–3", accent="#C9A84C")
    with c4:
        kpi_card("Licensed / Clear", m.licensed,
                 "On official register", accent="#50C88A")
    with c5:
        hint = f"+{m.risk_increased} risk up" if m.risk_increased else "Surfaced this run"
        kpi_card("New Alerts", m.new_entities, hint, accent="#3DA5E0")


def _render_priority_queue(df: pd.DataFrame, session) -> None:
    section_header("Priority Review Queue",
                   "Top entities by risk — focus here first")
...
def _render_summary(m: RunMetrics, df: pd.DataFrame) -> None:
    ...
    body = "".join(
        ...
        <div style="display:flex;justify-content:space-between;
            ...
            <span style="color:var(--text);font-size:12.5px;font-weight:700;">
                {value}
            </span>
        </div>
        """
    ...
    for _, r in dist.iterrows():
        ...
        bars.append(f"""
            <div style="display:flex;align-items:center;gap:10px;margin:6px 0;">
                <span style="width:90px;font-size:10px;font-weight:700;
                             text-transform:uppercase;color:var(--muted);">
                    {r['label']}
                </span>
                <div style="flex:1;height:8px;border-radius:999px;
                            background:rgba(128,128,128,0.12);overflow:hidden;">
                    <div style="height:100%;width:{width:.1f}%;
                                background:{r['color']};border-radius:999px;"></div>
                </div>
                <span style="width:30px;text-align:right;font-size:12px;
                             font-weight:700;color:var(--text);">
                    {r['count']}
                </span>
            </div>
        """)
    st.markdown(f'<div class="uae-card">{"".join(bars)}</div>',
