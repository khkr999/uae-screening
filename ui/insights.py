...
def _render_risk_chart(dist: pd.DataFrame) -> None:
    section_header("Risk Distribution",
                   "Counts per tier across the current run")
    if dist["count"].sum() == 0:
        empty_state("No risk data"); return

    chart = (
        alt.Chart(dist)
        .mark_bar(cornerRadiusEnd=6, size=30)
        .encode(
...
def _render_regulator_chart(regs: pd.DataFrame) -> None:
    section_header("Regulator Scope",
                   "Top regulators by entity count")
    if regs.empty:
    ...
    chart = (
        ...
        .encode(
            ...
            x=alt.X("count:Q", title="Count"),
            color=alt.value("#C9A84C"),
            tooltip=["regulator:N", "count:Q"],
        )
        .properties(height=240)
        .configure_view(stroke=None)
    )
