...
def kpi_card(label: str, value: str | int, hint: str = "", accent: str = "var(--accent)") -> None:
    st.markdown(
...
def risk_badge_html(level: int) -> str:
    ...
    return (f'<span class="uae-badge" style="background:{tier.accent_bg};'
            f'color:{tier.color};border-color:{tier.color}33;">{escape(tier.label)}</span>')
...
def error_state(title: str, detail: str = "") -> None:
    st.markdown(
        f'<div class="uae-card nohover" style="border-color:rgba(239,68,68,0.35);border-left:3px solid #EF4444;">'
...
def entity_card(row: pd.Series, on_open_key: str) -> None:
    ...
    action_html = f'<div class="uae-action-lbl" style="margin-top:8px;">{action}</div>' if action else ""
    rat_html    = (f'<div style="font-size:11px;color:var(--muted);margin-top:10px;'
    ...
    st.markdown(
        ...
        f'<div style="font-size:11px;color:var(--dim);margin-top:2px;font-family:\'IBM Plex Mono\',monospace;">'
